...
# Modell med bara intercept
model_intercept <- lm("crim ~ 1", data = train_set) 
# Koden neda är enligt caret, men det funkar ej att hitta AIC. Så kör koden ovan.
# model_intercept <- train(crim ~ ., data = train_set, 
#                          method = "lm")

# Forward selection, hittar att 2 variabler är bäst.
# Algorithmen testar att lägga olika variabler först i modellen, 
# en olika som andra variabel, osv. 
test_forward <- train(crim ~ ., data = train_set, 
                      method = "leapForward")

# nvmax visar att max 2 variabler var bäst. 
test_forward

# Visar att den bästa modellen är att lägga till "rad" först och sen "lstat"
summary(test_forward)

# Skattar modellen enligt forward selection.
forward_mod <- lm(crim ~ rad + lstat, train_set, method="lm")

# Om vi kolla på AIC så är den lägre för modellen med "rad + lstat" jämfört med 14 variabler.
AIC(model_intercept)
AIC_vec
AIC(forward_mod)
...
formula <- as.formula(paste("crim ~", paste(predictors_subset, collapse = "+")))
# formula <- as.formula("crim ~ rad + lstat")
...
ten_fold_ridgereg <- function(k_fold_index,train_set,lambda){
  ...
  for(i in 1:10){
    ...
    RMSE[i] <- sqrt(sum((test_data$crim - model$predict(test_data[,c(9,13)]))^2))
  }
  mean_RMSE <- mean(RMSE)
  return(mean_RMSE)
}
